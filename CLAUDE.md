...
대칭 판별 (`check_symmetry`) + 직선변 판별 기반:

```python
# 1순위: 좌우대칭 + 가로≥50 + 세로≥45 → 1장 (BACK)
# 2순위: 좌우대칭 + 가로≥50 + 20≤세로<45 → 1장 (BACK YOKE)
# 3순위: 가로≥25 + 세로≥40 + 세로직선(≥35cm) → 2장 (FRONT)
# 4순위: 좌우대칭 + 가로≥45 + 세로≤15 + 가로직선 → 1장 (BACK YOKE HEM)
# 5순위: 좌우대칭 + 가로≥50 + 세로≤10 + 평행선(85%) → 1장 (BACK BOTTOM)
# 6순위: 좌우대칭 + 가로≤25 + 세로≤15 → 4장 (FLAP)
# 7순위: 상하대칭 + 가로≤26 + 세로≤12 → 4장 (SLEEVE TAB)
# 8순위: 나머지 → 2장 (확인)
```

**직선변 판별 함수:**
- `check_vertical_straight_edge(poly, min_length_cm)`: 세로 직선변 감지 (≥min_length_cm)
- `check_horizontal_straight_edge(poly)`: 가로 직선변 감지
- `check_parallel_edges(poly, similarity_threshold)`: 상하단 평행선 감지 (유사도 85%)
...
10. **수량/구분 결정 로직 전면 개편** (2026-01-13)
    - 8가지 조건으로 세분화 (BACK, BACK YOKE, FRONT, BACK YOKE HEM, BACK BOTTOM, FLAP, SLEEVE TAB, 확인)
    - 새로운 직선변 판별 함수: `check_vertical_straight_edge()`, `check_horizontal_straight_edge()`, `check_parallel_edges()`
    - 상하대칭 조건 추가 (SLEEVE TAB)

11. **네스팅 시뮬레이션 사이즈별 벌수 설정** (2026-01-13)
    - 다중 사이즈 DXF: 사이즈별 개별 벌수 설정 가능 (예: S/2, M/3, L/2)
    - 재네스팅에도 동일 기능 적용
    - 단일 사이즈일 때는 기존 벌수 입력 사용

12. **네스팅 시각화 색상 로직 개선** (2026-01-13)
    - 단일 사이즈: 같은 패턴 = 같은 색상 (패턴별 구분)
    - 다중 사이즈: 같은 사이즈 = 같은 색상 (사이즈별 구분)

13. **사이즈 중첩 비교 개선** (2026-01-13)
    - pattern_group 기반 그룹화 (df 정렬과 무관하게 정확한 패턴 매칭)
    - 전역 크기 비율 적용 (global_max_dim): 모든 패턴 동일 스케일로 비교
    - 원본 DXF 위치 유지 (중심 정렬 제거)

14. **패턴 번호 동기화 수정** (2026-01-13)
    - df/patterns 리스트 동시 정렬 (원단별 정렬 시 동기화)
    - 상세리스트: 기본 사이즈 필터링 후 번호 순차 재설정 (1, 2, 3...)
    - 썸네일 그리드: 그레이딩 패턴도 순차 번호 표시
    - 드래그앤드롭 영역 높이 2배 확대 (min-height: 200px)
